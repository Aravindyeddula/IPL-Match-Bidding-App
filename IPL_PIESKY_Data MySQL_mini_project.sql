...
# 1.	Show the percentage of wins of each bidder in the order of highest to lowest percentage.
 ...
 select t1.bidder_id,t1.BIDDER_NAME , (sum(if(t3.bid_status='won',1,0))/count(*))*100 as Percentage_of_Wins
 from ipl_bidder_details t1
 join ipl_bidder_points t2
 on t1.BIDDER_ID=t2.BIDDER_ID
 join ipl_bidding_details t3
 on t2.BIDDER_ID = t3.BIDDER_ID
 group by  t1.bidder_id,t1.BIDDER_NAME
 order by Percentage_of_Wins desc;

# 2.	Display the number of matches conducted at each stadium with stadium name, city from the database.

select t1.stadium_id,stadium_name,count(MATCH_ID) matches_conducted,city
from ipl_stadium t1
join ipl_match_schedule t2
on t1.STADIUM_ID = t2.STADIUM_ID
group by t1.STADIUM_ID;

# 3.	In a given stadium, what is the percentage of wins by a team which has won the toss?

select t1.MATCH_ID,t3.STADIUM_NAME,t1.MATCH_WINNER,(sum(if(t1.match_winner=t1.toss_winner,1,0))/count(*))*100 as percent
from ipl_match t1
join ipl_match_schedule t2
on t1.MATCH_ID= t2.MATCH_ID
join ipl_stadium t3
on t2.STADIUM_ID=t3.STADIUM_ID
group by t2.STADIUM_ID   ;

# 4.	Show the total bids along with bid team and team name.

select t3.TEAM_NAME,t1.BID_TEAM,sum(t2.NO_OF_BIDS) `Total Bids` from 
ipl_bidding_details t1
join 
ipl_bidder_points t2
on t1.BIDDER_ID = t2.BIDDER_ID
join ipl_team t3
on t3.TEAM_ID=t1.BID_TEAM
group by t1.BID_TEAM;
...
select t1.TEAM_NAME,
sum(t2.MATCHES_PLAYED) as `total matches played`,
sum(MATCHES_WON) as `total matches won`,
sum(MATCHES_LOST) as `total matches lost`
from ipl_team t1
join ipl_team_standings t2
on t1.TEAM_ID = t2.TEAM_ID
group by t1.TEAM_ID;

# 7.	Display the bowlers for Mumbai Indians team.

select itp.player_id,player_name,itp.team_id,it.team_name,itp.player_role
from ipl_team it
join  ipl_team_players itp
on itp.team_id = it.team_id
join ipl_player ip
on itp.player_id = ip.player_id
where it.team_name = 'Mumbai Indians' 
and itp.player_role = 'Bowler' ; 

#8.	How many all-rounders are there in each team, Display the teams with more than 4 all-rounder in descending order.

select * ,count(t2.PLAYER_ROLE)
from ipl_team t1 
join ipl_team_players t2
on t1.team_id = t2.team_id
where t2.PLAYER_ROLE= 'all-rounder'
group by t2.TEAM_ID
having count(t2.PLAYER_ROLE)>4
order by count(t2.PLAYER_ROLE) desc;